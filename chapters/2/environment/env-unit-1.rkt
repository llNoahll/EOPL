...
(define-type Empty-Env  (List 'empty-env))
(define-type Extend-Env (List 'extend-env Symbol Any Env))
...
(define-unit env@
  ...
  (define extend-env*
    (λ (vars vals env)
      ...
      (cond [(null? vars)
             (error 'extend-env* "Bad vars: ~s" vars)]
            [(null? vals)
             (error 'extend-env* "Bad vals: ~s" vars)]
  ...
  (define apply-env
    (λ (env search-var)
      ...
      (cond [(empty-env? env)
            ...
            [(extend-env? env)
             (let ([saved-var : Symbol (cadr env)]
                   [saved-val : Any (caddr env)]
                   [saved-env : Env (cadddr env)])
  ...
  (define has-binding?
    (λ (env var)
      (cond [(empty-env? env) #f]
            [(eqv? var (cadr env)) #t]
            [else (has-binding? (cadddr env) var)]))))
