...
(let ()
  ...
  (add-primitive-proc! 'newref (λ [vals : DenVal *] : ExpVal
                                 (match vals
                                   [`(,val) (newref val)]
                                   [_ (error 'unary-func "Bad args: ~s" vals)])))
  (add-primitive-proc! 'deref  (λ [vals : DenVal *] : ExpVal
                                 (match vals
                                   [`(,ref)
                                    (if (ref? ref)
                                        (deref ref)
                                        (error 'deref "Bad arg: ~s" ref))]
                                   [_ (error 'unary-func "Bad args: ~s" vals)])))
  (add-primitive-proc! 'setref! (λ [vals : DenVal *] : ExpVal
                                  (match vals
                                    [`(,ref ,val)
                                     (if (ref? ref)
                                         (setref! ref val)
                                         (error 'setref! "Bad arg: ~s" ref))]
                                    [_ (error 'binary-func "Bad args: ~s" vals)])))
  (add-primitive-proc! 'ref? (λ [vals : DenVal *] : ExpVal
                               (match vals
                                 [`(,arg) (ref? arg)]
                                 [_ (error 'unary-func "Bad args: ~s" vals)])))
